...
def test_reverse_single_contig(
    ...
    assert (
        len(cds) == len(cds_after_reversal)
    ), "Reversal should not modify contig count"
    assert (
        len(sds) == len(sds_after_reversal)
    ), "Reversal should not modify stripe count"
    ...
    if contig_index < 0 or contig_index >= len(cds):
        ...
        assert cds[1+contig_index:] == cds_after_reversal[1 +
                                                          contig_index:], "Contigs before reversed should not be modified"
        assert cds[contig_index].direction != cds_after_reversal[contig_index].direction, "Requested contig should be reversed"
        assert (
            list(filter(
                lambda s: s.contig_descriptor.contig_id != contig_index,
                sds
            ))
            ==
            list(filter(
        ...
        ), "Stripes of other contigs should not be modified"
        assert all(
            [
                sd.contig_descriptor.direction != cds[contig_index].direction
                for sd in filter(
                    lambda s: s.contig_descriptor.contig_id == contig_index, sds_after_reversal
                )
            ]
        ), "All stripes of reversed contig should be reversed"
...
@pytest.mark.randomize(
    contig_lengths_bp=list_of(int, items=100), min_num=0, max_num=1000000)
@pytest.mark.randomize(
    start_contig_id=int, min_num=0, max_num=99, ncalls=15)
@pytest.mark.randomize(
    end_contig_id=int, min_num=0, max_num=99, ncalls=15)
@pytest.mark.randomize(
    contig_lengths_at_resolution_src=list_of(list_of(int, items=10), items=100), min_num=0, max_num=1000)
def test_reverse_contig_segment(
        log_resolutions,
        resolution_index,
        contig_directions,
        contig_lengths_bp,
    ...
    if end_contig_id < 0 or start_contig_id >= len(cds):
        ...
        assert cds[1+end_contig_id:] == cds_after_reversal[1+end_contig_id:], "Contigs before reversed segment should not be modified"
        assert all(
            (
                cd.contig_id != cdr.contig_id
                for cd, cdr in list(zip(reversed(cds[start_contig_id:end_contig_id]), cds_after_reversal[start_contig_id:end_contig_id]))
            )
        ), "Requested contig segment should have contig ids reversed"        
        assert all(
            (
                cd.direction != cdr.direction
                for cd, cdr in list(zip(reversed(cds[start_contig_id:end_contig_id]), cds_after_reversal[start_contig_id:end_contig_id]))
            )
        ), "Requested contig segment should be reversed"
        assert (
            list(filter(
                lambda s: s.contig_descriptor.contig_id not in contig_ids_to_be_reversed,
                sds
            ))
            ==
            list(filter(
        ...
        assert all(
            (
                sd.contig_descriptor.direction != cds[start_contig_id].direction
                for sd in filter(
                    lambda s: s.contig_descriptor.contig_id in contig_ids_to_be_reversed, sds_after_reversal
                )
            )
        ), "All stripes of reversed contig should be reversed"
